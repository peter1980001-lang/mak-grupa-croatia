// src/components/AquaCityLocation.jsx — slides 6–8
...
export default function AquaCityLocation() {
  ...
  useEffect(() => {
    ...
    tl.fromTo(headlineRef.current,
      { opacity: 0, y: 16 }, { opacity: 1, y: 0, ease: 'none', duration: 0.16 }, 0.28)
    tl.fromTo(bodyRef.current,
      { opacity: 0, y: 12 }, { opacity: 1, y: 0, ease: 'none', duration: 0.14 }, 0.42)
    return () => ScrollTrigger.getAll().forEach((t) => t.kill())
  }, [])

  return (
    <div data-section="aquacity-location">
      <CanvasSection name="aquacity-location" totalFrames={144} height="320vh">
        <div style={{
        ...
        <div style={{ position: 'absolute', top: '50%', left: '8%', transform: 'translateY(-50%)', maxWidth: '500px' }}>
          ...
          <ul ref={bodyRef} style={{ opacity: 0, listStyle: 'none', padding: 0, margin: 0 }}>
            {[
              'Prirodno jezero blizu urbanog središta',
              'Zagreb · Slovenija · Austrija',
              'Promet i vidljivost već postoje',
            ].map((item) => (
              <li key={item} style={{
