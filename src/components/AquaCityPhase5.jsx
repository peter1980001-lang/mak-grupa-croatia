...
export default function AquaCityPhase5() {
    ...
    ScrollTrigger.create({
      ...
      onEnter: () => {
        ...
        cardTimerRef.current = setTimeout(() => {
          gsap.to(cardRef.current, { opacity: 0, y: 40, x: -20, duration: CARD_FADE_OUT, ease: 'power2.in' })
        }, CARD_HOLD_MS)
      },
      onLeaveBack: () => {
  ...
  return (
    <div data-section="aquacity-phase5">
      <CanvasSection name="aquacity-phase5" totalFrames={189} height="320vh" zIndex={14} focusX={0.5} holdLastRatio={0.021}>
        <div style={{
        ...
        <div ref={cardRef} className="slide-card" style={{ position: 'absolute', bottom: '8%', left: '8%', maxWidth: '540px', textAlign: 'left', opacity: 0 }}>
          <GlassBox teal style={{ borderRadius: '16px', padding: '2.2rem 3rem' }}>
            <p style={{
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
              {['Plutajući bungalovi na jezeru', 'Glamping s premium uslugom', 'Produžen boravak gostiju', 'Noćenje uz jezero'].map((item) => (
                <li key={item} style={{
